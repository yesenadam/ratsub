...
srandset: SRAND NUM EOL {srand($2);} //this was changed to srandom, so not affecting crand at all. crand was the same every time.
//need a separate srandom i guess.
;
...
} | PLUS NUM  { //Colour addition   + 0.1  = + 0.1 0.1 0.1
    ColourAddIsUsed=1;
    Ss->ShapeCall[nss].ColourAddIsUsed=1;
    SetShapeCallCol($2/10.0,$2/10.0,$2/10.0);
} | MINUS NUM NUM NUM { //Colour subtraction   - 0.1 0.1 -1 NB must be " - "
    ColourAddIsUsed=1;
    Ss->ShapeCall[nss].ColourAddIsUsed=1;
    SetShapeCallCol(-$2/10.0,-$3/10.0,-$4/10.0);
} | MINUS NUM { //Colour subtraction   - 0.1 = - 0.1 0.1 0.1 NB must be " - "
    ColourAddIsUsed=1;
    Ss->ShapeCall[nss].ColourAddIsUsed=1;
    SetShapeCallCol(-$2/10.0,-$2/10.0,-$2/10.0);
...
/*    */
    ...
    else if (SC->EqualIsUsed) printf(" colR colG colB\n");
